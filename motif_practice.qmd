...
```{r}
motif_ts_results <- mimics_process(
...
- Create the spectrogram crops
  
```{r}
list_out <- feature_extraction(
  motif_ts_results = motif_ts_results,
  indices_output_folder = "output/indices-output-ap-mini",
  folder_extension = ".flac",
  cropped_images_path = "output/cropped"
)
```

- Create sound crops (optional)
...
```{r}
list_out <- find_matching_audio(
  list_out,
  "data/A2O-mini"
)

list_out_crops <- crop_audio(
  list_out,
  "output/sound-cropped"
)
```
...
- anthrophony could be car, airplane etc if you are interested in classifying
...
mimics_classify(list_out_crops, seed = 1234)

# For continuing classifying
continue_df <- read.csv("img-class-backup-2023-04-10.csv")
mimics_classify(continue_df)
```
