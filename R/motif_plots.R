#' Motif plots
#'
#' This function will use your indices data, and hime data, to generate plots and combined motif/index data frames (.csv files)
#'
...
motif_plots <- function(data_indices_all, outputfigures, himedatapath) {
        ...
        # these df will each get passed to the plot and TODO functions
        for (sites in total_sites) {
                for (month_id in total_months) {
                        ...
                        indicestimeplots(complete_ts = complete_ts,
                        ...
                        hime_indices(total_indices = total_indices,
...
indicestimeplots <- function(complete_ts, pathid, siteID, total_indices, Y.month_id) {
        ...
        plot_ts <- complete_ts %>% dplyr::select(position, all_of(total_indices)) %>%
                tidyr::pivot_longer(.,
                                    cols = 2:4,
                                    names_to = "index",
                                    values_to = "value")
        ...
        ggplot2::ggplot(plot_ts, aes(x = position, y = value)) +
                geom_line() +
                facet_wrap(. ~ index) +
                theme_classic() +
                theme(axis.text.x = element_blank())

        ggplot2::ggsave(file.path(pathid,
...
hime_indices <- function(total_indices, complete_ts, himedatapath, X.sites, X.month_id, figures) {
        for (index in total_indices) {
                complete_inter <-
                        dplyr::select(complete_ts, all_of(index), 4:ncol(complete_ts)) %>%
                ...
                hime_file_import_path <- file.path(himedatapath,
                                                       paste("Res_TS_",
                                                             index, "_",
                                                             X.month_id, "_",
                                                             X.sites,
                ...
                motif_results <-
                        ...
                        dplyr::mutate(., id = 1:as.numeric(count(.))) %>%
                        ...
                        remove_repeated(.) # remove repeated seems to be working
                ...
                ggsave(file.path(figures, paste(X.sites, X.month_id, index, "indicespertime.jpg", sep = "_")))
