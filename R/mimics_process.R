#' Combine motif and time series data and generate outputs
#'
#' @param my_indices_data Data frame produced by `time_series()` as input
#' @param outputfigures Character string; Path to folder where output figures
#' will be saved
#' @param himepath Character string; Path to folder where hime motif results
#' were saved when using the `run_hime()` function.
#' @param threshold Numeric between 0 and 1; Threshold for removing overlapping
#' motifs. Default is 0.9.
#' @return
#' @export mimics_process
mimics_process <- function(
    my_indices_data,
    outputfigures,
    himepath,
    threshold) {
...
#' Check if indices data is of valid input format.
#'
#' Helper function for `mimics_process()`.
#'
#' @keywords internal
