...
feature_extraction <- function(
  ...
  make_csvs(
    motif_complete = motif_complete_df,
  ...
  for (siteID in siteIDs) {
    ...
    for (index_string in indices_string) {
      ...
      for (row in 1:nrow(img_prep_site_index)) {
        ...
        path_to_image <- grep(
          pattern = pattern_match_image_list,
          x = image_list,
          value = TRUE
        )
...
make_csvs <- function(motif_complete, outputpath) {
  ...
  for (siteID in sites) {
    ...
    write.csv(wtData, get_data_path(
      outputpath,
      paste(siteID, "_", "_wavelet.csv", sep = "")
    ), row.names = F)

    write.csv(samples, get_data_path(
      outputpath,
      paste(siteID, "_", "_LabelsSample.csv", sep = "")
    ), row.names = F)
